...
class MY_Model extends CI_Model
{
    ...
    */
     ...
     * @return array
     */
     public function fread_as_array($file)
    {
        $data=array();
        $fp=fopen($file, 'r');
        while (!feof($fp))
     ...
     * @return string
     */
     public function fread_as_string($file)
    {
       $data='';
        $fp=fopen($file, 'r');
        while (!feof($fp))
    ...
    public function database_backup($file,$path,$download=false)
    {
        if (!file_exists($path.$file))
        {
            $prefs = array(
            'tables'        => array(),//array('table1', 'table2'),   // Array of tables to backup.
            'ignore'        => array(),                     // List of tables to omit from the backup
            'format'        => 'gzip',                       // gzip, zip, txt
            'filename'      => 'ci_linked_e_m.sql',              // File name - NEEDED ONLY WITH ZIP FILES
    ...
    public function max_value($column,$where='')
    {
        //$this->_database->select('COUNT(*) AS counted_rows',FALSE);
        ...
        //$result = $this->_database->get('students')->row();  
        //echo $result->time; 
    }
