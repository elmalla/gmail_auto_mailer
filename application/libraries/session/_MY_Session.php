...
class MY_Session extends CI_Session{
  ...
  public function __construct($params = array()) {
    ...
    {
      $CI->$key = (isset($params[$key])) ? $params[$key] : $CI->config->item($key);
    }

    if ($CI->encryption_key == '')
    {
    ...
    if ($CI->sess_encrypt_cookie == TRUE)
    {
    ...
    if ($this->sess_use_redis == TRUE) {
      $CI->redis = new Redis();
      $CI->redis->connect($CI->config->item('redis_host'), $CI->config->item('redis_port'));
      //$this->redis->setOption(Redis::OPT_SERIALIZER, Redis::SERIALIZER_PHP); //php5.4 can not work
      $CI->redis->setOption(Redis::OPT_PREFIX, 'sessions:');
    }
    ...
    if ($CI->sess_expiration == 0)
    {
      $CI->sess_expiration = (60*60*24*365*2);
    }

    // Set the cookie name
    $CI->sess_cookie_name = $CI->cookie_prefix.$CI->sess_cookie_name;
